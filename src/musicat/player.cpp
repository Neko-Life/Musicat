...
size_t Player::remove_track(size_t pos, size_t amount, const size_t to) {
    if (!pos || (!amount && (long)to == -1)) return 0;
    ...
    if ((long)to != -1 && to > pos) amount = to - pos;
    size_t max = siz - pos;
...
size_t Manager::remove_track(dpp::snowflake guild_id, size_t pos, const size_t amount, const size_t to) {
