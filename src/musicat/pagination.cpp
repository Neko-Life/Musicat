...
pages_t::edit_cb (const dpp::confirmation_callback_t &cb, size_t new_current)
{
    ...
    if (cb.is_error ())
        {
            fprintf (stderr, "[ERROR PAGINATE EDIT_CB]:\nmes: %s\ncode: %d\nerr:\n", cb.get_error ().message.c_str (),
                     cb.get_error ().code);
...
pages_t::edit (size_t c, const dpp::interaction_create_t &event)
{
                ...
                [] (const dpp::confirmation_callback_t &e)
                    {
                        if (e.is_error ())
                            {
                                util::log_confirmation_error (e, "[pages_t::edit event.reply ERROR] "
                                                                 "(current == c)");
                            }
                    });
    ...
    if (!has_permissions (dpp::find_guild (this->message->guild_id), &this->client->me, dpp::find_channel (this->message->channel_id),
    ...
    event.from ()->creator->interaction_response_create (event.command.id, event.command.token, reply,
                                                         [this, c] (const dpp::confirmation_callback_t &cb) { this->edit_cb (cb, c); });
...
void
update_page (const dpp::snowflake &msg_id, const std::string &param, const dpp::interaction_create_t &event)
{ //, dpp::message* msg)
    if (param.empty ())
        return;
    ...
    if (a == paginated_messages.end ())
        {
            ...
            event.reply (util::response::str_mention_user (event.command.usr.id)
                         + util::rand_item (std::vector<std::string>{ "The book for this message is missing!",
    ...
    if (param == "n")
        ...
        {
            dpp::component input = util::response::create_short_text_input ("Page number to jump to:", "j");

            dpp::interaction_modal_response modal ("page_queue", "Jump to page");
    ...
    if (invalid_char != 0)
        {
            event.reply (
                ...
                + util::rand_item (std::vector<std::string>{ "That isn't a number!", "maybe some ppl never got to school, such sad :(",
                                                             "wrong number, guess again!", "ur father", "maybe maybe maybe",
    ...
    try
        ...
        {
            event.reply (util::response::str_mention_user (event.command.usr.id)
                         + util::rand_item (std::vector<std::string>{ "no", "don't be ridiculous", "smfh", "NO wtf", "don't",
...
get_inter_reply_cb (const dpp::interaction_create_t &event, bool paginate, dpp::cluster *client, std::vector<dpp::embed> embeds,
            ...
            if (paginate || has_v)
                {
                    event.get_original_response (
                        [client, embeds, has_v, storage_data] (const dpp::confirmation_callback_t &cb2)
                            {
                                if (cb2.is_error ())
                                    {
                                ...
                                const bool debug = get_debug_state ();
                                if (debug)
                                    std::cerr << "[PAGINATE GET_INTER_REPLY_CB] PAGINATED ID: " << m->id
    ...
    while (i != paginated_messages.end ())
        {
            ...
            if (clear || (t - (time_t)L) > ONE_HOUR_SECOND)
                {
                    if (debug)
                        std::cerr << "Deleting " << ++d << ": " << i->second.message->id << '\n';
...
void
_construct_desc (std::deque<player::MCTrack> &queue, std::deque<player::MCTrack>::iterator i, std::string &desc, size_t &id, size_t &count,
                 size_t &qs, dpp::embed &embed, const std::string &title, std::vector<dpp::embed> &embeds, uint64_t &totald,
                 std::shared_ptr<player::guild_player_t> guild_player)
{
    if (i == queue.begin ())
        {
            ...
            desc += "Current track: [" + mctrack::get_title (*i) + "](" + mctrack::get_url (*i) + ")"
                    + std::string (!prog.status ? std::string (" [") + format_duration (prog.current_ms) + "/"
    ...
    else
        {
            ...
            desc += std::to_string (id) + ": [" + mctrack::get_title (*i) + "](" + mctrack::get_url (*i) + ")"
                    + std::string (dur ? std::string (" [") + format_duration (dur) + "]" : "") + " - <@" + std::to_string (i->user_id)
                    + ">\n";
    ...
    if ((count && !(count % 10)) || id == qs)
        {
            embed.set_title (title).set_description (desc.length () > 2048 ? "Description too long, pagination is on the way!" : desc);
...
void
reply_paginated_playlist (const dpp::interaction_create_t &event, std::deque<player::MCTrack> queue, const std::string &title,
                          const bool edit_response)
{
    ...
    for (auto i = queue.begin (); i != queue.end (); i++)
        {
            _construct_desc (queue, i, desc, id, count, qs, embed, title, embeds, totald, guild_player);
        }
    ...
    if (!edit_response)
        ...
        event.edit_response (msg, paginate::get_inter_reply_cb (event, paginate, event.from ()->creator, embeds));
