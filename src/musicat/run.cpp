...
int run(int argc, const char* argv[])
{
    ...
    client.on_log(
            [&dpp_on_log_handler](const dpp::log_t& event)
            {
    ...
    client.on_form_submit(
            [&player_manager](const dpp::form_submit_t& event)
            {
                ...
                if (event.custom_id == "modal_p")
                {
                    if (event.components.size())
                    {
                        ...
                        if (comp.custom_id.find("que_s_track") != std::string::npos)
                        {
                            ...
                            std::thread dlt(
                                    [comp, prepend_name, player_manager, dling, fname, guild_id, from](const dpp::interaction_create_t event, yt_search::YTrack result)
                                    {
    ...
    client.on_autocomplete(
            [&player_manager](const dpp::autocomplete_t& event)
            {
    ...
    client.on_interaction_create(
            [&player_manager](const dpp::interaction_create_t& event)
            {
                ...
                if (cmd == "hello") 	return command::hello::slash_run(event);
                if (cmd == "why")		return event.reply("Why not");
                if (cmd == "hi")		return event.reply("HIII");
                if (cmd == "invite") 	return command::invite::slash_run(event);
                if (cmd == "support") 	return event.reply("https://www.discord.gg/vpk2KyKHtu");
                if (cmd == "repo") 	return event.reply("https://github.com/Neko-Life/Musicat");
                if (cmd == "pause") 	return command::pause::slash_run(event, player_manager);
                if (cmd == "skip") 	return command::skip::slash_run(event, player_manager); // add 'force' arg, save djrole within db
                if (cmd == "play") 	return command::play::slash_run(event, player_manager);
                if (cmd == "loop") 	return command::loop::slash_run(event, player_manager);
                if (cmd == "queue") 	return command::queue::slash_run(event, player_manager);
                if (cmd == "autoplay") 	return command::autoplay::slash_run(event, player_manager);
                if (cmd == "move") 	return command::move::slash_run(event, player_manager);
                if (cmd == "remove") 	return command::remove::slash_run(event, player_manager);
                if (cmd == "bubble_wrap") 	return command::bubble_wrap::slash_run(event);
                if (cmd == "search") 	return command::search::slash_run(event);
                if (cmd == "playlist") 	return command::playlist::slash_run(event, player_manager);
                if (cmd == "stop") 	return command::stop::slash_run(event, player_manager);
                if (cmd == "interactive_message") 	return command::interactive_message::slash_run(event);
                if (cmd == "join") 	return command::join::slash_run(event, player_manager);
                if (cmd == "leave") 	return command::leave::slash_run(event, player_manager);
    ...
    client.on_voice_track_marker(
            [&player_manager](const dpp::voice_track_marker_t& event)
            {
