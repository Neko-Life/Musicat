...
on_channel_update (dpp::cluster *client)
{
    client->on_channel_update (
        [] (const dpp::channel_update_t &event)
            {
                if (!event.updated.id)
                    return;
                ...
                // convenience?
                guild_player = player::manager::get_player (guild_id);
            ...
            on_channel_update_skip_to_rejoin:
                ...
                player::manager::full_reconnect (event.from (), guild_id, channel_id, channel_id);
