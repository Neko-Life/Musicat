...
{
inline const command::command_handler_t *command_handlers = command::get_slash_command_handlers ();
        ...
        [] (const dpp::slashcommand_t &event) -> dpp::task<void>
            {
                ...
                auto status = command::handle_command ({ cmd, command_handlers, event });
