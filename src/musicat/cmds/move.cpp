    ...
    namespace move {
        namespace autocomplete {
            void track(const dpp::autocomplete_t& event, string param, player_manager_ptr player_manager, dpp::cluster& client) {
                ...
                {
                    string nm = std::to_string(i.first) + ": " + i.second;
        ...
        void slash_run(const dpp::interaction_create_t& event, player_manager_ptr player_manager) {
            ...
            mc::get_inter_param(event, "to", &to);
            ...
            if (fr != to)
            {
                ...
                if (a != p->queue.at(1).title() || b != p->queue.back().title()) try
                {
                    player_manager->update_info_embed(event.command.guild_id);
                }
                catch (...) {}
            }
            event.reply(string("Moved ") + t.title() + " to position " + std::to_string(to));
        }
    }
}
