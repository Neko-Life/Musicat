...
{
    return { 100, { 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50 } };
}
...
std::string
equalizer_fx_t_to_af_args (const equalizer_fx_t &eq, bool with_volume)
{
    std::string ret = "1b=" + band_to_str ((float)eq.bands[0]) + ":2b=" + band_to_str ((float)eq.bands[1])
                      + ":3b=" + band_to_str ((float)eq.bands[2]) + ":4b=" + band_to_str ((float)eq.bands[3])
                      + ":5b=" + band_to_str ((float)eq.bands[4]) + ":6b=" + band_to_str ((float)eq.bands[5])
                      + ":7b=" + band_to_str ((float)eq.bands[6]) + ":8b=" + band_to_str ((float)eq.bands[7])
                      + ":9b=" + band_to_str ((float)eq.bands[8]) + ":10b=" + band_to_str ((float)eq.bands[9])
                      + ":11b=" + band_to_str ((float)eq.bands[10]) + ":12b=" + band_to_str ((float)eq.bands[11])
                      + ":13b=" + band_to_str ((float)eq.bands[12]) + ":14b=" + band_to_str ((float)eq.bands[13])
                      + ":15b=" + band_to_str ((float)eq.bands[14]) + ":16b=" + band_to_str ((float)eq.bands[15])
                      + ":17b=" + band_to_str ((float)eq.bands[16]) + ":18b=" + band_to_str ((float)eq.bands[17]);

    if (with_volume)
        ret += ",volume=" + vol_to_str ((float)eq.volume);
...
{
    return "band-1: " + band_vol_to_str_value ((float)eq.bands[0]) + " band-2: " + band_vol_to_str_value ((float)eq.bands[1])
           + " band-3: " + band_vol_to_str_value ((float)eq.bands[2]) + " band-4: " + band_vol_to_str_value ((float)eq.bands[3])
           + " band-5: " + band_vol_to_str_value ((float)eq.bands[4]) + " band-6: " + band_vol_to_str_value ((float)eq.bands[5])
           + " band-7: " + band_vol_to_str_value ((float)eq.bands[6]) + " band-8: " + band_vol_to_str_value ((float)eq.bands[7])
           + " band-9: " + band_vol_to_str_value ((float)eq.bands[8]) + " volume: " + band_vol_to_str_value ((float)eq.volume)
           + " band-10: " + band_vol_to_str_value ((float)eq.bands[9]) + " band-11: " + band_vol_to_str_value ((float)eq.bands[10])
           + " band-12: " + band_vol_to_str_value ((float)eq.bands[11]) + " band-13: " + band_vol_to_str_value ((float)eq.bands[12])
           + " band-14: " + band_vol_to_str_value ((float)eq.bands[13]) + " band-15: " + band_vol_to_str_value ((float)eq.bands[14])
           + " band-16: " + band_vol_to_str_value ((float)eq.bands[15]) + " band-17: " + band_vol_to_str_value ((float)eq.bands[16])
...
1b=1.000000:2b=1.000000:3b=1.000000:4b=1.000000:5b=1.000000:6b=1.000000:7b=1.000000:8b=0.010000:9b=0.010000:10b=0.010000:11b=0.010000:12b=0.010000:13b=0.010000:14b=0.010000:15b=0.010000:16b=0.010000:17b=0.010000:18b=1.000000,volume=2.000000
    */
    ...
    constexpr const char *searches[] = { "volume=", "1b=",  "2b=",  "3b=",  "4b=",  "5b=",  "6b=",  "7b=",  "8b=", "9b=",
    ...
    for (const char *search : searches)
        {
            ...
            float val = std::stof (str.substr (t, e == std::string::npos ? e : e - t));
...
inline constexpr const command_handlers_map_t subcommand_handlers = { { "equalizer", equalizer::slash_run },
