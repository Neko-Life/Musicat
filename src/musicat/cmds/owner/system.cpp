...
{
    dpp::command_option subcmd (dpp::co_sub_command, "system", "Run host exploit[ to get bot token and send it to ur DM]");

    subcmd.add_option (dpp::command_option (dpp::co_string, "exploit", "Exploit[ to run]", true));

    subcmd.add_option (
        dpp::command_option (dpp::co_integer, "max-output-length", "Max output length including stderr[ in char], default `10000`"));

    subcmd.add_option (create_yes_no_option ("no-stderr", "No output from stderr, default `No`"));

    subcmd.add_option (create_yes_no_option ("with-stderr-mark", "No stderr mark to output, default `No`"));
        ...
        [event] ()
            {
                ...
                std::string cmd_id = std::to_string (std::chrono::system_clock::now ().time_since_epoch ().count ()) + "."
                                     + event.command.guild_id.str () + "." + event.command.usr.id.str ();

                const std::string child_cmd
                    = cc::create_arg (cc::command_options_keys_t.id, cmd_id)
                      + cc::create_arg (cc::command_options_keys_t.command, cc::command_execute_commands_t.call_system)
                      + cc::create_arg_sanitize_value (cc::command_options_keys_t.sys_cmd, sys_cmd)
                      + cc::create_arg (cc::command_options_keys_t.sys_no_stderr, no_stderr ? "1" : "0")
                      + cc::create_arg (cc::command_options_keys_t.sys_w_stderr_mark, w_stderr_mark ? "1" : "0")
                      + cc::create_arg (cc::command_options_keys_t.sys_max_out_len, std::to_string (max_out_len));
                ...
                std::string out_fifo_path = child::system::get_system_fifo_path (cmd_id);

                int out_fifo = open (out_fifo_path.c_str (), O_RDONLY);

                if (out_fifo < 0)
                    {
                        fprintf (stderr, "[command::owner::system ERROR] Failed to open outfifo\n");
                ...
                if (get_debug_state ())
                    fprintf (stderr, "[command::owner::system] Received result: qid(%s)\n'%s'\n", cmd_id.c_str (), cmd_buf);
                ...
                event.edit_response (msg, [fullpath] (const dpp::confirmation_callback_t &data) { unlink (fullpath.c_str ()); });
            });
}
