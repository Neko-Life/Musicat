...
{
    dpp::command_option subcmd (dpp::co_sub_command, "set_avatar", "Set Musicat avatar");

    subcmd.add_option (dpp::command_option (dpp::co_attachment, "image", "Image[ to set as avatar]", true));

    slash.add_option (subcmd);
}

std::vector<std::string> valid_avatar_formats = { "image/jpeg", "image/png", "image/gif" };

void
slash_run (const dpp::slashcommand_t &event)
{
    std::string musicat_username = (event.from () && event.from ()->creator) ? event.from ()->creator->me.username : "";

    if (musicat_username.empty ())
        return event.reply ("`[ERROR]` For some reason I can't remember my name, why do I "
        ...
        [event, img_id, musicat_username] ()
            {
                ...
                if (!valid_type)
                    {
                        return event.edit_response ("Valid avatar image format are only JPEG, PNG and GIF");
                    }
                ...
                if (res_str.empty () || res_body.empty ())
                    {
                        ...
                        return event.edit_response ("Something went wrong when fetching attachment...");
                    }
                ...
                if (img_len > MAX_IMG_SIZE)
                    {
                        return event.edit_response ("File cannot be larger than 10240.0 kb.");
                    }
                ...
                std::string body = nlohmann::json ({ { "username", musicat_username },
                                                     { "avatar",
                                                       // data:
                                                       "data:"
                                                           // image/jpeg
                ...
                if (patch_err)
                    {
                        return event.edit_response ("`[ERROR]` Status " + std::to_string (s_code));
                    }
