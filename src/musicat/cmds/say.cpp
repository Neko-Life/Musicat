...
parse_msg_source (const std::string &content)
{
    ...
    size_t end = 0;
    if (content.find (prefix) != 0 || (end = content.find (":>", psiz)) == std::string::npos)
        return ret;
...
{
    dpp::command_option channel_opt (dpp::co_channel, "to", "Channel to send the message");

    using ct = dpp::channel_type;
    channel_opt.channel_types = { ct::CHANNEL_TEXT, ct::CHANNEL_ANNOUNCEMENT, ct::CHANNEL_ANNOUNCEMENT_THREAD,
                                  /* ct::CHANNEL_FORUM, ct::CHANNEL_MEDIA, */
                                  ct::CHANNEL_PRIVATE_THREAD, ct::CHANNEL_PUBLIC_THREAD, ct::CHANNEL_VOICE, ct::CHANNEL_STAGE };

    return dpp::slashcommand ("say", "Just say it! Ima say it if ur too scared to say it", sha_id)
        .add_option (dpp::command_option (dpp::co_string, "content", "Content of the message", true))
                ...
                if (to_channel)
                    {
                        ...
                        if (i == event.command.resolved.channels.end ())
                            return event.reply ("`[ERROR]` Unable to retrieve channel");

                        sndmsg = dpp::message (i->first, content);

                        event.thinking (false,
                                        [sndmsg, event] (const dpp::confirmation_callback_t &thinking_res)
                                            {
                                                if (thinking_res.is_error () || !event.from () || !event.from ()->creator)
                                                    return;

                                                event.from ()->creator->message_create (
                                                    sndmsg,
                                                    [event] (const dpp::confirmation_callback_t &create_res)
                                                        {
                                                            if (create_res.is_error ())
                                                                // !TODO: actually check musicat and member
                                                                // permission before sending
                                                                return event.edit_response ("Failed sending message, please check "
