...
get_register_obj (const dpp::snowflake &sha_id)
{
    return dpp::slashcommand ("remove", "Remove [tracks] from the queue", sha_id)
        .add_option (dpp::command_option (dpp::co_integer, "track", "Remove [this] track", true))
        .add_option (dpp::command_option (dpp::co_integer, "amount", "Amount [of track] to remove [starting from this track]"))
        .add_option (dpp::command_option (dpp::co_integer, "to", "Remove [up to this] track [position]"));
}
...
slash_run (const dpp::slashcommand_t &event)
{
    ...
    const size_t ret = player::manager::remove_track (event.command.guild_id, (size_t)fr, (size_t)amount, (size_t)to);
    if (ret)
        {
            server::ws::player::publish_queue (event.command.guild_id);

            event.edit_response ("Removed " + std::to_string (ret) + util::join (ret != 1, " track", "s"));

            auto queue2 = player::manager::get_queue (event.command.guild_id);
            if (queue2.size () < 2U || mctrack::get_title (queue2.at (1)) != mctrack::get_title (f_t)
                || mctrack::get_title (queue2.back ()) != mctrack::get_title (l_t))
