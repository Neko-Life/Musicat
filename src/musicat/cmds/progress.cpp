...
dpp::message
_reply_embed (player::MCTrack &current_track, const int64_t &current_ms, const int64_t &duration)
{
    dpp::message msg;
    dpp::embed embed;
    ...
    msg.add_component (dpp::component ().add_component (
        dpp::component ().set_label ("Update").set_id ("progress/u").set_type (dpp::cot_button).set_style (dpp::cos_primary)));
...
slash_run (const dpp::slashcommand_t &event)
{
    ...
    try
        {
            _processed_t value = _process_command (event);
