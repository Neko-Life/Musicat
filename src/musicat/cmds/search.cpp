...
get_register_obj (const dpp::snowflake &sha_id)
{
    return dpp::slashcommand ("search", "Search [for tracks]", sha_id)
        .add_option (dpp::command_option (dpp::co_string, "query", "Search [this]", true).set_max_length (150));
}

dpp::interaction_modal_response
modal_enqueue_searched_track ()
{
    dpp::component input = util::response::create_short_text_input ("Input track number to add to playback queue:", "que_s_track");
...
{
    dpp::component input = util::response::create_short_text_input ("Input track number to add to top of queue:", "que_s_track_top");
...
{
    dpp::component input1 = util::response::create_short_text_input ("Input track number to add to playback queue:", "que_s_track");
    dpp::component input2 = util::response::create_short_text_input ("Input position to slip into playback queue:", "que_s_track_slip");
...
search_entry_append_track_info (const player::MCTrack &track)
{
    ...
    return " [" + mctrack::get_length_str (track) + "] - " + mctrack::get_channel_name (track);
}
        ...
        [event] ()
            {
                ...
                try
                    {
                        res = mctrack::fetch ({ query, YDLP_DEFAULT_MAX_ENTRIES, false });
                    }
                catch (std::exception &e)
                    {
                        std::cerr << "[command::search::slash_run ERROR] " << event.command.guild_id << ':' << e.what () << '\n';
                ...
                for (auto &t : tracks)
                    {
                        ...
                        desc += std::string ("`") + std::to_string (cn) + "`: [" + tit_char
                                + (tit.length () > tit_char.length () ? "..." : "") + "](" + mctrack::get_url (t) + ")"
                        ...
                        if (count == 10 || cn == pl_siz)
                            {
                                ...
                                emb.set_title (query).set_description (desc.length () > 2048 ? "Description exceeding char limit. rip"
                                                                                             : desc);
