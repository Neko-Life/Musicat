        ...
        void slash_run(const dpp::interaction_create_t& event, player_manager_ptr player_manager)
        {
            ...
            if (qarg > -1)
            {
                ...
                case queue_modify_t::m_clear_left: {
                    ...
                    try
                    {
                        auto vc = mc::get_voice_from_gid(event.command.guild_id, player_manager->sha_id);
                        {
                            std::lock_guard<std::mutex> lk(p->q_m);
                            for (auto i = p->queue.begin(); i != p->queue.end(); i++)
                            {
                                ...
                                if (cont) continue;
                                if (mc::vector_find(&l_user, i->user_id) == l_user.end())
                    ...
                    event.reply(std::to_string(usiz)
                        ...
                        + std::to_string(rmed) + " track" + string(rmed > 1 ? "s" : "")
                    );
            ...
            if (paginate)
            {
                mc::paginate::add_pagination_buttons(&msg);
            }

            event.reply(msg, mc::paginate::get_inter_reply_cb(event, paginate, player_manager->cluster, embeds));
        }
    }
}
