...
{
    return ftp.guild_player->vibrato_f;
}

static int
get_d (const filters_perquisite_t &ftp)
{
    return ftp.guild_player->vibrato_d;
}
...
{
    return modulation::setup_subcommand (slash, { "vibrato", "Well, vibrate[ playback audio]", "Vibrate" });
}
