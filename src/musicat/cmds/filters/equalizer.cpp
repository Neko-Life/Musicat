...
inline constexpr const char *eq_options[][2] = {
    { "band-1", "Set 65Hz band gain" },     { "band-2", "Set 92Hz band gain" },     { "band-3", "Set 131Hz band gain" },
    { "band-4", "Set 185Hz band gain" },    { "band-5", "Set 262Hz band gain" },    { "band-6", "Set 370Hz band gain" },
    { "band-7", "Set 523Hz band gain" },    { "band-8", "Set 740Hz band gain" },    { "band-9", "Set 1047Hz band gain" },
    { "volume", "Set filter volume" },      { "band-10", "Set 1480Hz band gain" },  { "band-11", "Set 2093Hz band gain" },
    { "band-12", "Set 2960Hz band gain" },  { "band-13", "Set 4186Hz band gain" },  { "band-14", "Set 5920Hz band gain" },
    { "band-15", "Set 8372Hz band gain" },  { "band-16", "Set 11840Hz band gain" }, { "band-17", "Set 16744Hz band gain" },
    { "band-18", "Set 20000Hz band gain" },
};

inline constexpr const size_t eq_options_size = (sizeof (eq_options) / sizeof (*eq_options));
...
setup_subcommand (dpp::slashcommand &slash)
{
    ...
    for (size_t i = 0; i < igoal; i++)
        {
            dpp::command_option eqsubcmd (dpp::co_sub_command, "equalizer", "Apply 18 band equalizer");

            eqsubcmd.add_option (dpp::command_option (dpp::co_string, "action", "What you wanna do?", false)
                                     .add_choice (dpp::command_option_choice ("Set", action_t_e.set))
                                     .add_choice (dpp::command_option_choice ("Balance", action_t_e.balance))
                                     .add_choice (dpp::command_option_choice ("Reset", action_t_e.reset)));

            size_t jgoal = (i + 1) * argpc;
            for (size_t j = i * argpc; j < jgoal && j < eq_options_size; j++)
                {
                    eqsubcmd.add_option (dpp::command_option (dpp::co_integer, eq_options[j][0], eq_options[j][1], false)
...
inline constexpr const command_handlers_map_t action_handlers
    = { { "", show }, { action_t_e.set, set }, { action_t_e.balance, balance }, { action_t_e.reset, reset }, { NULL, NULL } };
...
balance (const dpp::slashcommand_t &event)
{
    ...
    constexpr const char *new_equalizer = "1b=0.5:2b=0.5:3b=0.5:4b=0.5:5b=0.5:6b=0.5:7b=0.5:8b="
