...
int
create_resume_timer (const dpp::snowflake &user_id, const dpp::snowflake &user_voice_channel_id, dpp::discord_voice_client *vc,
                     long long min_delay)
{
    if (!vc)
        return 1;

    if (player::manager::is_manually_paused (vc->server_id))
        {
            return 2;
        }
    ...
    resume_timer_t rt = { util::get_current_ts (), user_id, vc->server_id, vc->channel_id, user_voice_channel_id, min_delay };

    if (get_debug_state ())
        {
            std::cerr << "[player::timer::create_resume_timer] "
                         "Creating resume_timer_t: ts("
                      << rt.ts << ") uid(" << rt.uid << ") svcid(" << rt.svcid << ") vcsid(" << rt.svid << ") vcvcid(" << rt.vcid << ")\n";
        }
            ...
            if (debug)
                {
                    std::cerr << "[player::timer::check_resume_timers] "
                              ...
                              << now << ") ts(" << i->ts << ") diff(" << diff << ") uid(" << i->uid << ") svcid(" << i->svcid << ") vcsid("
                              << i->svid << ") vcvcid(" << i->vcid << ")\n";
                }

            auto guild_player = player::manager::get_player (server_id);

            if (guild_player)
                player::manager::unpause (guild_player->get_voice_client (), server_id, true);
