...
namespace musicat {
	...
	/**
	 * @brief Get all user playlist, PGresult pointer must be freed using finish_res()
	 *
	 * @param user_id
	 * @param type
	...
	/**
	 * @brief Get one user playlist with id `name`, PGresult pointer must be freed using finish_res()
	 *
	 * @param user_id
	 * @param name
	 * @param type
	...
	/**
	 * @brief Get saved guild playback, PGresult pointer must be freed using finish_res()
	 *
	 * @param guild_id
	 ...
	 * @param loop_mode
	 * @return ExecStatusType -1 if guild_id is 0, -2 if all the three last param is null,
	 * 			-3 if failed to create new table, PGRES_COMMAND_OK on success
	 */
	ExecStatusType
	...
	/**
	 * @brief Get guild player config, PGresult pointer must be freed using finish_res()
	 *
	 * @param guild_id
	 * @return std::pair<PGresult*, ExecStatusType> status PGRES_FATAL_ERROR on failure, else PGRES_COMMAND_OK
	 */
	std::pair<PGresult*, ExecStatusType>
	    get_guild_player_config(const dpp::snowflake& guild_id);
	
	/**
	 * @brief Parse player config contained in PGresult, this function does not free the PGresult pointer.
	 *
	 * @param res
	 * @return std::pair<player_config, int> code 0 if non-default value returned, else 1
	 */
