...
struct YTrack
{
    ...
    int get_thumb (nlohmann::json &d, const std::string &url,
                   const bool debug);
 ...
 * The container must have `raw` property with a type of nlohmann::json which
 * stores the data
 *
 * @tparam T
...
template <typename T>
void
get_data (const std::string url, T *container,
          const std::string search = "var ytInitialData = ")
{
    ...
    if (sI == std::string::npos)
        {
            ...
            fprintf (stderr,
                     "Not a valid youtube search query (or youtube update, yk "
                     "they like to change stuffs)\nvar_start: %ld\n",
                     sI);
            return;
        }
    ...
    if (eI == std::string::npos)
        {
            ...
            fprintf (
                stderr,
                "Not a valid youtube search query (or youtube update, yk they "
                "like to change stuffs)\nvar_start: %ld\nvar_end: %ld\n",
                sI, eI);
            return;
        }
...
bool transverse (nlohmann::json &j,
                 const std::initializer_list<const char *> path);
void loop_json (nlohmann::json &d,
